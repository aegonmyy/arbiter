...
Meaning-based matching is used only for open-ended and factual tasks, where two
prompts that mean the same thing want the same answer. Code, math and structured
tasks use lexical near-identity, so "calculate 37 * 24" is never served the cached
...
drift detection (section 7) now fires on a move that is *either* large in relative terms
