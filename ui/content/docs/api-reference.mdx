...
| `GET /`, `/app`, `/start`, `/docs` | The web app (see [interface.md](/docs/interface)). |

`/v1/chat/completions`, `/v1/feedback`, `/v1/reset` and `/v1/simulate-price`
require an `Authorization: Bearer <key>` (see
[Client authentication](/docs/integration#client-authentication)); a missing or
...
| `cascaded_from` | If the cascade escalated, the cheap model it escalated from (else `null`). |
...
| `latency_capped` | Whether an `arbiter_max_latency` ceiling removed any model. |
...
| `failover_from` | Models that errored before this one served, or `null`. |
...
- **Response headers** available immediately: `X-Arbiter-Task`,
  ...
  headers are sent, so the serving model is reported in the trailing event
  instead (where it is always accurate).
...
Record human thumbs up/down on a routed answer. The strongest quality signal the router
has; it overrides the model judge over a few votes. Requires auth.
...
block. A `422` is returned for an unknown model, missing task, or invalid rating.
The response echoes the running vote counts for that task and model.
...
([interface.md](/docs/interface)). If the interface has not been built, `/` serves
