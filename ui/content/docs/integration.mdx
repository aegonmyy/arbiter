---
title: "Integration"
description: "Run it, plug in, and the key / auth model."
---
...
Playground, docs) see [interface.md](/docs/interface).
...
`arbiter_no_cache`) are covered in [developers.md](/docs/developers#3-routing-controls-budget-latency-cache).
...
response (see [api-reference.md](/docs/api-reference#post-v1chatcompletions)) for
...
- **Minted at signup.** A user gives an email at `/start` (or `POST /v1/register`)
  ...
  Playground request. See [interface.md](/docs/interface).
