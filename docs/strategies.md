...
appears again with the judge; see section 5.)
...
computed against (see section 6).
...
have no ground truth here and get a neutral score, deferred to the judge (section 5).
...
reports cost per response - which is also why the same signal powers section 7.
...
**Problem.** Quarantine (section 8) skips a broken model on the *next* request, but the
...
**Approach.** `POST /v1/feedback` folds thumbs up/down into routing quality with enough
weight (`FEEDBACK_WEIGHT`) to override the judge over a few votes
(`policy.py`). Separately, drift detection (section 7) now fires on a move that is
