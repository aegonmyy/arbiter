...
per task (see [strategies.md](strategies.md#6-measuring-savings--from-headers-not-list-prices)).
...
## GET /v1/key and POST /v1/key/{action}
...
`POST /v1/key/pause`, `/resume`, `/revoke` change the status. A paused key still
...
A failed call is **not** recorded into the policy, so a transient upstream error
...
([interface.md](interface.md)). If the interface has not been built, `/` serves
