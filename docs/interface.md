...
| `/app` | The dashboard. Sectioned into tabs: Overview, Playground, API key, Models. |
| `/docs` | This documentation, rendered with Fumadocs. |
| `/v1/*` | The API (see [api-reference.md](api-reference.md)). |
...
- **Overview** - the headline stats (calls routed, total spend, average cost per
  call, task types learned), a live routing feed, secondary stats (pool size, how
  requests were classified, price shifts caught), price alerts, a short
  how-it-works, and an integration card that links into the Playground. Per-call
  cost is measured from the runtime's headers; there is no savings figure on
  display, since in free-routing mode it stays at zero.
- **Playground** - type or pick an example prompt and route it live. A budget
  slider sets a max cost per request and previews, in real time, which models fit
  that budget for the current prompt. The result shows a plain-English summary
  plus the task, difficulty, mode, quality, cost, latency, cache status and
  eligible-model count, each with a short hint, and the actual answer - and calls
  out when a request escalated (cascade) or failed over. Every run is a real
  request through the runtime.
...
Every screen reads from the API endpoints in [api-reference.md](api-reference.md)
